...
object Parameters {
    ...
    const val OFF_ROUTE_M               = 30_000.0  // m, cross-track distance that triggers OFF_ROUTE
    const val OFF_ROUTE_CONSECUTIVE     = 2         // consecutive GOOD fixes beyond OFF_ROUTE_M
    const val OFF_ROUTE_CLEAR_S         = 60.0      // s, back inside threshold before clearing
    const val CROSS_TRACK_DECAY_TAU_S   = 600.0     // s, time constant for cross-track decay to route
    ...
    const val SIGMA_D_INITIAL_M         = 20_000.0  // m, prior cross-track sigma (predicted-only)
    ...
    const val UNCERTAINTY_SIGMAS        = 2.0       // sigma multiplier for drawn uncertainty
