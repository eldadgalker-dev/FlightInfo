...
נוצר אוטומטית ממסכי העזרה של האפליקציה (גרסה 5.3-beta34).
...
ההקלטה מתחילה רק בלחיצה על כפתור REC במפה (פועם באדום בזמן הקלטה, עיגול אדום כשלא) ונעצרת בלחיצה נוספת, 10 דקות אחרי הנחיתה, או אחרי 3 שעות על הקרקע. בזמן הקלטה הטיסה כותבת קובץ CSV בטלפון (Android/data/com.galker.flightinfo/files/logs) עם ההערכה ונתוני GPS, ברומטר וג׳ירוסקופ גולמיים, פעם בשנייה. דבר אינו מועלה. "שתף יומן אחרון" שולח את הקובץ דרך כל אפליקציה שתבחר; יומן מטיסה אמיתית הוא הדרך הטובה ביותר לכייל את מנוע ההערכה.  
  
הצגה חוזרת של טיסה:  
1. באפליקציה: הגדרות > יומני טיסה > הקש על יומן > הפעל. המפה מנגנת אותו ב־30/120/600× דרך המנוע העכשווי.  
2. בכל מחשב עם רשת: פתח https://eldadgalker-dev.github.io/FlightInfo/replay/ וגרור את קובץ ה־CSV (שתף אותו קודם מהאפליקציה במייל או ל־Drive).  
3. במחשב ללא רשת: הורד פעם אחת את docs/replay/index.html מעמוד הפרויקט ("הילוך חוזר במחשב" > שמור קובץ) ופתח בלחיצה כפולה; גרור את ה־CSV. אותו מסך כמו בטלפון; התצלום דורש רשת, כל השאר עובד ללא רשת.
